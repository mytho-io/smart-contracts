    ...
    PausableUpgradeable 
{
    ...
    mapping(address user => mapping(uint256 milestone => uint256 count)) private userBadgeCounts;
    ...
    // Events
    event BadgeMinted(address indexed to, uint256 indexed tokenId, uint256 indexed milestone);
    event MilestoneURIUpdated(uint256 indexed milestone, string uri);
    event BoostSystemUpdated(address indexed oldBoostSystem, address indexed newBoostSystem);
    
    // Errors
    error OnlyBoostSystem();
    error InvalidMilestone();
    error MilestoneURINotSet();
    error BoostSystemNotSet();
    ...
    function mintBadge(
        ...
        if (boostSystem == address(0)) revert BoostSystemNotSet();
        if (bytes(milestoneURIs[milestone]).length == 0) revert MilestoneURINotSet();
     ...
     */
    function getTokenMilestone(uint256 tokenId) external view returns (uint256 milestone) {
     ...
     */
    function getMilestoneURI(uint256 milestone) external view returns (string memory uri) {
     ...
     */
    function getUserTotalBadgeCount(address user) external view returns (uint256 totalCount) {
     ...
     */
    function isMilestoneURISet(uint256 milestone) external view returns (bool isSet) {
